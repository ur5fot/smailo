...
└── server/                 # Бэкенд на Express + TypeScript
    └── src/
        ...
        │   └── app.ts            # GET/POST /api/app/:hash — доступ, чат, история чата
...
- Чат в приложении: клиент → `POST /api/app/:hash/chat` → AI-сервис (фаза chat) → опциональное обновление UI и/или обновление памяти
- История чата: клиент → `GET /api/chat?sessionId=home-<userId>` / `GET /api/app/:hash/chat` → последние 20 сообщений восстанавливаются при загрузке страницы
- Память AI: после каждого ответа в приложении AI может вернуть `memoryUpdate` — краткие заметки (до 2000 символов), которые сохраняются в `apps.notes` и автоматически передаются AI при следующих разговорах
