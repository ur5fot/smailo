...
- [x] Табы страниц в AppEditor с возможностью: переключить, добавить новую, удалить, переименовать
- [x] Drag-and-drop табов для изменения порядка страниц
- [x] При добавлении страницы: диалог с id, title, icon
- [x] Переключение single-page ↔ multi-page: если добавляется вторая страница, конвертировать uiComponents в pages
- [x] Property editor для страницы: title, icon, id (read-only после создания)
- [x] Написать тесты для page management
- [x] Запустить тесты — должны проходить перед следующим таском

### Task 12: AI промпты — обновить документацию по layout
- [x] Обновить `BRAINSTORM_SYSTEM_PROMPT` и `IN_APP_SYSTEM_PROMPT` в `aiService.ts` — добавить документацию по `layout` полю
- [x] AI должен генерировать layout для новых приложений (разумные defaults: cards по 6 колонок, tables/charts full-width, buttons по 3-4 колонки)
- [x] Обновить примеры в промптах
- [x] Написать тесты для валидации AI-генерируемых layout
- [x] Запустить тесты — должны проходить перед следующим таском
...
- [x] Обновить `README.md` — описание визуального редактора
- [x] Обновить `README.ru.md` — описание визуального редактора
- [x] Обновить `CLAUDE.md` — добавить секцию про редактор, layout, новые файлы
- [x] Обновить `docs/roadmap-v2.md` — отметить Этап 7 как реализованный
