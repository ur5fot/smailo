...
- [x] Проверить что существующие приложения без showIf/styleIf работают без изменений (обратная совместимость)
- [x] Проверить edge cases: несуществующий dataKey в showIf, деление на 0 в условии, null значения
- [x] Запустить полный тест suite (`npm test --workspace=server`)
