...
- [ ] одностраничное приложение (без `pages`) работает без изменений
- [ ] многостраничное приложение: вкладки отображаются, URL меняется при переходе
- [ ] прямая ссылка на `/:userId/:hash/:pageId` открывает нужную страницу
- [ ] несуществующий `pageId` в URL → редирект на первую страницу
- [ ] `computedValue` компоненты работают корректно на всех страницах
- [ ] запустить `npm test --workspace=server` — все тесты проходят
- [ ] запустить `npm test --workspace=client` — все тесты проходят
