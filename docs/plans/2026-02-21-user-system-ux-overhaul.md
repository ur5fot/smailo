...
- [x] В `server/src/routes/chat.ts`: при создании приложения (фаза `created`) читать `userId` из тела запроса и сохранять в apps.userId
- [x] `GET /api/app/:hash` — включать `userId` в ответ (клиенту нужно знать для построения URL)
- [x] Обновить `BRAINSTORM_SYSTEM_PROMPT` — убрать из него создание сессий, т.к. теперь userId приходит снаружи (сессия в чате остаётся sessionId, а userId — отдельный параметр)
...
- [x] В `InputBar.vue` добавить быстрые кнопки-числа (1, 2, 3) которые появляются под полем ввода когда последнее сообщение ассистента содержит нумерованный список (`/^\d+\./m`)
- [x] При нажатии кнопки-цифры — вставлять число в поле ввода и сразу отправлять
- [x] Работает и в UserView (создание) и в AppView (редактирование)

### Task 11: Обновить CLAUDE.md и READMEs

- [x] Обновить `CLAUDE.md` — новые маршруты, структура views, users таблица, новые компоненты
- [x] Обновить `README.md` (EN) — новая архитектура пользователей
- [x] Обновить `README.ru.md` (RU) — то же самое
