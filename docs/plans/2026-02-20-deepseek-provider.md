...
### Task 5: Verify
- [x] убедиться что сервер стартует без ошибок (`npm run dev` в server)
- [x] проверить что при `AI_PROVIDER=anthropic` работает как раньше
- [x] проверить что при `AI_PROVIDER=deepseek` вызывается DeepSeek API
